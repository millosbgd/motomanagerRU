...
@Component({
  ...
  template: `
    ...
    <!-- Modal: TXT izveštaj -->
    <p-dialog
      [(visible)]="txtReportVisible"
      [modal]="true"
      [closable]="true"
      [draggable]="false"
      [style]="{width: '700px', maxWidth: '96vw'}"
      styleClass="dark-dialog"
      (onHide)="txtReportVisible = false">
      <ng-template pTemplate="header">
        <span style="font-size:16px; font-weight:600; color:#f1f5f9;">TXT izveštaj</span>
      </ng-template>
      <div style="display:flex; flex-direction:column; gap:12px;">
        <textarea [(ngModel)]="txtReportContent"
          style="width:100%; height:400px; background:#0f172a; border:1px solid #334155; border-radius:8px; color:#e2e8f0; padding:12px; font-family:monospace; font-size:12px; line-height:1.5; outline:none; resize:vertical; box-sizing:border-box;"
        ></textarea>
        <div style="display:flex; justify-content:flex-end; gap:8px;">
          <button class="btn" style="background:#334155; color:#94a3b8;" (click)="txtReportVisible = false">Zatvori</button>
          <button class="btn btn-primary" (click)="copyTxtFromModal()">
            <i [class]="txtCopied ? 'pi pi-check' : 'pi pi-copy'"></i>
            {{ txtCopied ? 'Kopirano!' : 'Kopiraj' }}
          </button>
        </div>
      </div>
    </p-dialog>
...
export class ServiceOrdersComponent implements OnInit {
  ...
  copyTxtFromModal() {
    navigator.clipboard.writeText(this.txtReportContent).then(() => {
      this.txtCopied = true;
      setTimeout(() => this.txtCopied = false, 2500);
    });
  }
